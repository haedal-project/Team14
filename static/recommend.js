...
function withshowbest() { // 반려동물 동반 가능
    $.ajax({
        type: 'GET',
        url: '/api/recommend?sample_give=샘플데이터',
        data: {},
        success: function (response) {
            let puppy = response['puppy']
            for (let i = 0; i < puppy.length; i++) {
                let name = puppy[i]["title"]
                let x = puppy[i]["x"]
                let y = puppy[i]["y"]
                let like = puppy[i]["like"]
                ...
                if (like == 1) {
                    ...
                    temp_html = `<a href="#" class="list-group-item list-group-item-action flex-column align-items-start" style="margin: 10px;">
                                <div class="d-flex w-100 justify-content-between">
                          <h5 class="mb-1">${name}<small class="text-muted"></small></h5> <!-- 안에 평점 -->
                                    <span><button type="button" onclick="plus('${name}')" id="like_button_white" class="btn btn-danger" >ෆ</button> + ${like}</span>
                                </div>
                            </a>`
                }
                    ...
                    let callback = function (result, status) {
                        ...
                        if (status === kakao.maps.services.Status.OK) {
                            ...
                            var marker = new kakao.maps.Marker({
                                ...
                                title: `${name} \n${address}`, // 마커의 타이틀, 마커에 마우스를 올리면 타이틀이 표시됩니다
...
let id ="test3"
function showbest() { // 추천 장소 띄우기 (중복제거)
    $.ajax({
        type: 'GET',
...
function clickPlaceMarker(title, lat, lng, address) {
    $('#info-place-name').text(title) // 리뷰 등록할 때 장소명
    $('#info-place-address').text(address) // 주소 (상세정보)
...
function makeReview() {
    ...
    let like = 0
    let with_puppy = ""
    var withpp = document.getElementById('customCheck1')
    ...
    $.ajax({
        type: "POST",
        url: "/placereview",
        data: {name_give: name, review_give: review, rating_give: like, 'lat_give': lat, 'lng_give': lng, 'with_give': with_puppy},
        success: function (response) {
            alert(response["msg"]);
